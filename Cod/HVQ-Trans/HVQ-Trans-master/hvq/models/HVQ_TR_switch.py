...
from ..tools.distributed import distributed as dist_fn
...
class Quantize(nn.Module):
    def __init__(self, dim, n_embed, decay=0.99, eps=1e-5):
        ...
        # Inicializar prototipos de manera aleatoria
    ...
    def forward(self, input):
        ...
        # Ajustar prototipos durante el entrenamiento
        if self.training:
            embed_onehot_sum = embed_onehot.sum(0)
            embed_sum = flatten.transpose(0, 1) @ embed_onehot

            #LAST ERROR HERE
            # Not all backends include all_reduce https://pytorch.org/docs/stable/distributed.html

            embed_onehot_sum = embed_onehot_sum
            embed_sum = embed_sum
...
class HVQ_TR_switch(nn.Module):
    def __init__(
        ...
        # Encoder
        self.enc = efficientnet_b4(pretrained=False, outblocks=[1,5,9,21],outstrides=[2,4,8,16])#CAMBIO (preentrenamiento 3 bandas)
    ...
    def forward(self, inputs):
        ...
        # Enocde
        output_encoder = self.encoder(feature_tokens,mask=None,pos=pos_embed)  
        output_encoder = rearrange(output_encoder, 'n l b c -> n b l c')

        # VQ
        quant_list, diff, _ = self.encode(output_encoder, label)    
        quant_list = rearrange(quant_list, 'n b l c -> n l b c')    
        decode_pos_embed = torch.cat([pos_embed, pos_embed],dim=2)  
    ...
    def encode(self, input_list, label):
        ...
        for q_i in range(quant_4.size()[0]):
            # Corregir, formato distinto del esperado (deben ser tensores)

            if isinstance(label, list) and all(isinstance(l, str) for l in label):
                label = torch.tensor([int(l) for l in label])
            if isinstance(label[q_i], torch.Tensor):
                tmp_label = label[q_i].cpu().numpy()
            else:
                raise TypeError("Expected label[q_i] to be a tensor, but got {}".format(type(label[q_i])))
