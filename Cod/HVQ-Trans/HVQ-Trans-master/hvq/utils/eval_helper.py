...
def merge_together(save_dir):
    ...
    pred_imgs = np.asarray(pred_imgs)                  # N x 5 x H x W
...
class EvalImage:
    ...
    def encode_mask(self, masks):
        N, _, _ = masks.shape
        masks = (masks.reshape(N, -1).sum(axis=1) != 0).astype(int)  # (N, ) #np.int deprecated
        return masks
...
def performances(fileinfos, preds, masks, config):
    ...
    for clsname in clsnames:
        ...
        if config.get("auc", None):
            for metric in config.auc:
                ...
                auc, threshold = eval_method.eval_auc()
                # Se guarda el umbral para la prediccion de cada pixel
                if evalname == "pixel":
                    threshold_pix = threshold
    ...
    return ret_metrics, threshold_pix
